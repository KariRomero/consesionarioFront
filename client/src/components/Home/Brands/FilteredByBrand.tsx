...
const FilteredByBrand: React.FC<{ brandId: string }> = ({ brandId }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (isMobileView) setCardsToShow(1);
        ...
        setCardsToShow(4);
        setIsCarousel(!isMobileView && (brand?.vehiculos?.length || 0) > 4);
      }
    };
    handleResize();
    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize);
  }, [brand?.vehiculos?.length]);
  ...
  const next = () => {
    setCurrentIndex((prev) => (prev + 1) % (brand?.vehiculos?.length || 1));
  };

  const prev = () => {
    setCurrentIndex((prev) => (prev - 1 + (brand?.vehiculos?.length || 1)) % (brand?.vehiculos?.length || 1));
  };
  ...
  }  if (!brand?.vehiculos?.length) return null;

  const showHint = isMobile && brand.vehiculos.length > 1;
  const showCenteredSingle = isMobile && brand.vehiculos.length === 1;

  return (
    <div className="relative w-full px-4 sm:px-10 py-10">
      <h1 className="text-center text-2xl lg:text-3xl font-semibold pb-4">
        Estás viendo la marca {brand.nombre}
      </h1>
      ...
      >
        {(isCarousel || isMobile)
          ? (brand?.vehiculos || []).map((v: Vehiculo) => (
              <div
                key={v.id}
                className={
                  isMobile
                    ? (brand?.vehiculos?.length || 0) > 1
                      ? "min-w-[90%] pr-4"
                      : "min-w-[90%]"
                    : "min-w-[25%] px-2"
                }
