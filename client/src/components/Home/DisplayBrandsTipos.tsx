...
const DisplayBrandsTipos = ({ element }: DisplayBrandsTiposProps) => {
    ...
    const [selected, setSelected] = useState<string | null>(null);
    const [isAutoScrolling, setIsAutoScrolling] = useState(true); // Controla el desplazamiento automático
    const [isMobile, setIsMobile] = useState<boolean>(false); // Detectar si es mobile
    const carouselRef = useRef<HTMLDivElement>(null);
    const scrollTimeoutRef = useRef<NodeJS.Timeout | null>(null); // Referencia al temporizador
    const startX = useRef(0); // Para el desplazamiento táctil
    ...
    useEffect(() => {
        setIsMobile(window.innerWidth <= 768); // Ajusta el valor de 768px según tus necesidades
    }, []);
        ...
        if (currentCarousel) {
            ...
            const touchMoveHandler = (e: TouchEvent) => {
                if (e.touches.length === 1) {
                    ...
                    currentCarousel.scrollLeft += deltaX;
                    startX.current = e.touches[0].clientX;
                }
            };
            ...
            if (isMobile) {
                currentCarousel.addEventListener('touchstart', touchStartHandler, {
                    passive: false,
                });
                currentCarousel.addEventListener('touchmove', touchMoveHandler, {
                    passive: false,
                });
            }
            ...
            return () => {
                ...
                currentCarousel.removeEventListener('wheel', wheelHandler);
                if (isMobile) {
                    currentCarousel.removeEventListener('touchstart', touchStartHandler);
                    currentCarousel.removeEventListener('touchmove', touchMoveHandler);
                }
                currentCarousel.removeEventListener('scroll', handleScrollEnd); // Limpia el listener de scroll
            };
        }
    }, [element?.length, handleWheel, isAutoScrolling, isMobile]);
